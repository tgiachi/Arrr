...
public class SmtpSinkPluginTests
{
    ...
    public void SetUp()
    {
        _sender = new FakeMailSender();
        _sink   = new SmtpSinkPlugin(_sender);
    }
    ...
    {
        var ctx = new FakeSinkContext(configFactory: _ => new SmtpSinkConfig
        {
    ...
    public async Task ConsumeAsync_SingleMode_SendsOneEmailPerNotification()
    {
        ...
        await _sink.ConsumeAsync(new Notification(Guid.NewGuid(), "rss", "Title 1", "Body 1", DateTimeOffset.UtcNow, null), cts.Token);
        await _sink.ConsumeAsync(new Notification(Guid.NewGuid(), "rss", "Title 2", "Body 2", DateTimeOffset.UtcNow, null), cts.Token);
    ...
    {
        _sink = new SmtpSinkPlugin(_sender, digestInterval: TimeSpan.FromMilliseconds(100));
        var ctx = new FakeSinkContext(configFactory: _ => new SmtpSinkConfig
        {
        ...
        await _sink.ConsumeAsync(new Notification(Guid.NewGuid(), "rss", "T1", "B1", DateTimeOffset.UtcNow, null), cts.Token);
        await _sink.ConsumeAsync(new Notification(Guid.NewGuid(), "rss", "T2", "B2", DateTimeOffset.UtcNow, null), cts.Token);
        await _sink.ConsumeAsync(new Notification(Guid.NewGuid(), "rss", "T3", "B3", DateTimeOffset.UtcNow, null), cts.Token);
    ...
    {
        _sink = new SmtpSinkPlugin(_sender, digestInterval: TimeSpan.FromMinutes(60));
        var ctx = new FakeSinkContext(configFactory: _ => new SmtpSinkConfig
        {
            Mode = SmtpDeliveryMode.Digest,
            From = "from@example.com",
