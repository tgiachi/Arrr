...
public class HealthcheckPluginTests
{
    ...
    {
        await StartAndPoll(new() { Endpoints = [] });
    ...
    public void SetUp()
    {
        _handler = new();
        _eventBus = new();
        _plugin = new(_handler, new FakeTimeProvider(DateTimeOffset.UtcNow));
    }
    ...
    private static HealthcheckConfig OneEndpoint(
        ...
        => new()
        {
            Endpoints =
            [
                new() { Url = url, Name = name, ExpectedStatusCodes = expectedCodes ?? [] }
            ],
