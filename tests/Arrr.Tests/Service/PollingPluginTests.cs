...
public class PollingPluginTests
{
        ...
        return Task.Run(
            async () =>
            {
                while (!ct.IsCancellationRequested)
                {
                    ...
                    catch { /* host swallows plugin errors */ }
    ...
    {
        var plugin = new FakePollingPlugin(interval: TimeSpan.FromMilliseconds(10));
    ...
    public async Task PollAsync_ExceptionOnFirstCall_LoopContinues()
    {
        var plugin = new FakePollingPlugin(
            ...
            throwOnCall: call => call == 1 ? new InvalidOperationException("boom") : null
        );
        var bus = new EventBusService();
        using var cts = new CancellationTokenSource();
        await bus.StartAsync(cts.Token);
        ...
        Assert.That(plugin.PollCount, Is.GreaterThanOrEqualTo(2),
    ...
    public async Task PollAsync_Cancellation_StopsLoop()
    {
        ...
        Assert.That(plugin.PollCount, Is.EqualTo(countAtStop),
